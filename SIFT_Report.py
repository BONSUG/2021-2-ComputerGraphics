...
def SIFT(src):
    ...
    for i in range(len(keypoints)):
        ...
        for k in range(len(orient_hist)):
            if (np.max(orient_hist)>orient_hist[k])&(orient_hist[k]>(0.8*np.max(orient_hist))) :
                keypoints.append(KeyPoint(x,y,keypoints[i].size,float(k*10),keypoints[i].octave,keypoints[i].class_id))
    ...
    for i in range(len(keypoints)):
        ...
        for row in range(-8, 8):
            ...
            for col in range(-8, 8):
                ...
                new_Angle=angle[p_y,p_x]
                descriptor_angle= (((new_Angle-keypoints[i].angle)//45)+8)%8
                orient_hist[int(descriptor_angle)] = magnitude[p_y,p_x] * gaussian_weight
                descriptors[i,(8*(((row+8)//4)*4+((col+8)//4)))+int(descriptor_angle)]+=orient_hist[int(descriptor_angle)]
